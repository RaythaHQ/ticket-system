...
public class ExceptionsMiddleware
{
    ...
    public static RequestDelegate ErrorHandlerDelegate(string pathBase, IWebHostEnvironment env)
    {
        return async (HttpContext context) =>
        {
            ...
            if (path.Value?.ToLower().StartsWith($"{pathBase}/app/api") == true)
            {
                byte[] errorBytes;
            ...
            else
            {
                ...
                if (
                    path.Value?.ToLower().StartsWith($"{pathBase}/admin") == true
                    && !path.Value.ToLower().StartsWith($"{pathBase}/app/api")
                    && !path.Value.ToLower().StartsWith($"{pathBase}/admin/error")
                )
                {
                    ...
                    context.Response.Redirect($"{pathBase}/admin/error/{statusCode}");
                }
                else
                {
