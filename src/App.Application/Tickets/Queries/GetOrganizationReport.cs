...
public class GetOrganizationReport
{
    ...
    public class Handler : IRequestHandler<Query, QueryResponseDto<OrganizationReportDto>>
    {
        ...
        )
        {
            ...
            var ticketsInPeriod = await _db.Tickets
                .AsNoTracking()
                .Where(t => t.CreationTime >= startDate && t.CreationTime <= endDate)
                .ToListAsync(cancellationToken);

            var allCurrentTickets = await _db.Tickets
                .AsNoTracking()
                .ToListAsync(cancellationToken);

            var totalTicketsCreated = ticketsInPeriod.Count;
            var totalTicketsResolved = ticketsInPeriod.Count(t => t.ResolvedAt.HasValue && t.ResolvedAt.Value >= startDate);
            var totalTicketsClosed = ticketsInPeriod.Count(t => t.ClosedAt.HasValue && t.ClosedAt.Value >= startDate);
            var currentOpenTickets = allCurrentTickets.Count(t => t.Status != TicketStatus.CLOSED && t.Status != TicketStatus.RESOLVED);
            var currentUnassignedTickets = allCurrentTickets.Count(t => t.AssigneeId == null && t.Status != TicketStatus.CLOSED);

            var totalSlaBreaches = ticketsInPeriod.Count(t => t.SlaBreachedAt.HasValue && t.SlaBreachedAt.Value >= startDate);
            var totalSlaMet = ticketsInPeriod.Count(t => t.SlaStatus == SlaStatus.COMPLETED);
            var slaWithResult = totalSlaBreaches + totalSlaMet;
            var overallSlaComplianceRate = slaWithResult > 0 ? (double)totalSlaMet / slaWithResult * 100 : 100;
            ...
            double? avgResolutionTime = resolvedWithTimes.Any() ? resolvedWithTimes.Average() : null;
            double? medianResolutionTime = null;
            if (resolvedWithTimes.Any())
            {
                var sorted = resolvedWithTimes.OrderBy(h => h).ToList();
                int mid = sorted.Count / 2;
                medianResolutionTime = sorted.Count % 2 == 0
            ...
            {
                var teamTickets = ticketsInPeriod.Where(t => t.OwningTeamId == team.Id).ToList();
                ...
                return new TeamSummaryReportDto
                {
                    ...
                    OpenTickets = teamTickets.Count(t => t.Status != TicketStatus.CLOSED && t.Status != TicketStatus.RESOLVED),
                    SlaComplianceRate = teamSlaTotal > 0 ? Math.Round((double)teamMet / teamSlaTotal * 100, 2) : 100
                };
            }).ToList();

            return new QueryResponseDto<OrganizationReportDto>(new OrganizationReportDto
            {
                ...
                AverageResolutionTimeHours = avgResolutionTime.HasValue ? Math.Round(avgResolutionTime.Value, 2) : null,
                MedianResolutionTimeHours = medianResolutionTime.HasValue ? Math.Round(medianResolutionTime.Value, 2) : null,
